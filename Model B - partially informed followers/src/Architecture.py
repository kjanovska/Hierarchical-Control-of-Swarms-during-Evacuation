...
class Map:
    ...
    """
    def initialize(self):
        if self.params.map == "building1_4" and (self.params.scenario=="class" or self.params.scenario=="random"):
        ...
        if self.params.scenario == "random":
            self.load_agents_random(a)
        else:
            self.load_agents_file(a)
        self.print_map()
        self.assign_swarms()

    def load_agents_file(self, number_of_agents):
        self.leaders = self.load_leaders('scenarios/'+ str(self.params.map) +'_' + self.params.scenario + '_1D.txt')
        for leader in self.leaders:
            self.place_agent(leader)

        self.followers = self.load_followers('scenarios/'+ str(self.params.map) +'_'+self.params.scenario+'_F' + str(number_of_agents) + '.txt')
        for follower in self.followers:
            self.place_agent(follower)

    def load_agents_random(self, number_of_agents):
        for i in range(0, 9):
            a = 100 - self.params.informed_leaders
            b = random.randint(1, 100)
            inf = True
            ...
            leader = LeadingAgent(self.params.leader_sight, i, self.params.min_swarm_denominator, inf,
                                  self.params.leader_conflict_distance, self.exits)
            leader.position = self.find_empty_spot()
            leader.find_exit()
            self.leaders.append(leader)
            self.place_agent(leader)
        for j in range(0,number_of_agents - 9):
            follower = FollowingAgent(self.params.follower_sight, self.params.follower_obedience)
            follower.position = self.find_empty_spot()
            follower.known_exit = random.choice(copy.deepcopy(self.exits))
            ...
            for follower in self.followers:
                if not follower.evacuated:
                    ...
                    if not follower.evacuated:
                        ...
                        self.move(follower, where_to_move, 'F') # MOZNA JE PROBLEM, ZE SE TOHLE DEJE I KDYZ JE BLIZKO EXITU? NEVIM VUBEC
